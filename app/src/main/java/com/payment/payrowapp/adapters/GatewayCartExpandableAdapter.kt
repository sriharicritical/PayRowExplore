...
class GatewayCartExpandableAdapter(private val ring: MediaPlayer?,
                                   val sharedPreferenceUtil: SharedPreferenceUtil,
                                   private val context: Context,
                                   private val categoryList: List<Category>,
                                   private val productMap: MutableMap<String, ArrayList<Service>>,
                                   private val onQuantityChanged: (Service, Boolean) -> Unit,
    ...
    override fun getChildView(
        ...
        val product = getChild(groupPosition, childPosition) as Service
           ...
           ring?.start()
            if (product.quantity > 0) {
                ...
                if (sharedPreferenceUtil.getVATCalculator()) {
                    val payRowVATAmount = product.unitPrice.toFloat() / 100.0f * Constants.VAT_PER
                    product.transactionAmount -= payRowVATAmount + product.unitPrice
                } else {
