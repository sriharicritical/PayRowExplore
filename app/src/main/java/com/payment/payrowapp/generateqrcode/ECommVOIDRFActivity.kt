...
class ECommVOIDRFActivity : BaseActivity() {
    ...
    private lateinit var binding: ActivityEcommVoidrfactivityBinding
    var ring: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
      //  setContentView(R.layout.activity_ecomm_voidrfactivity)
        binding = ActivityEcommVoidrfactivityBinding.inflate(layoutInflater)
        ...
        if (bundle?.getString("mode") != null) {
            if (bundle.getString("mode") == "Refund") {
        ...
        if (status.equals(
            ...
            binding.tvStatus.text = getString(R.string.transaction_successful)
        } else /*if (status.equals("DENIED BY RISK", ignoreCase = true) || status.equals(
                "HOST TIMEOUT",
                ignoreCase = true
                ...
                "CANCELED",
                ignoreCase = true
            ) || status.equals("Pending", ignoreCase = true) || status.equals(
                "NOT VOIDED",
                ignoreCase = true
            )
        )*/ {
            ...
            binding.tvStatus.text = getString(R.string.payment_failed)
        }
        //set amount and va
        // tvBatchNo.text = bundle?.getString("Amount")
        val totalAmount = bundle?.getString("totalAmount")
        binding.tvTOTAmount.text =  "AED " +  ContextUtils.formatWithCommas(totalAmount!!.toDouble())//"AED " + bundle?.getString("totalAmount")
                    ...
                    if (optionSelected == "WhatsApp") {
                        ...
                        if (etEmail.text?.isNotEmpty() == true) {
                            ...
                            paymentSuccessfulActivityViewModel?.sendURLLiveData()
                                ?.observeOnce(this@ECommVOIDRFActivity) {
                                    bottomSheetDialog.dismiss()
                                    startActivity(
                                        Intent(
                                            this,
